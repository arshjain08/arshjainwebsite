...
export default function ProjectPage({ params }: any) {
  ...
  useEffect(() => {
    async function resolveParams() {
      ...
      const foundProject = projectsData.projects.find(p => p.id === resolved.slug);
    ...
    async function loadMarkdown() {
      try {
        ...
        if (res.ok) {
          ...
          if (isMounted) setMarkdown(`# ${project.title}\n\n${project.description}`);
        }
      } catch {
        if (isMounted) setMarkdown(`# ${project.title}\n\n${project.description}`);
      } finally {
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        >
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            <span
              className={`px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${
                ensuredProject.status === 'completed'
                  ? 'bg-green-100 text-green-700'
                  : 'bg-yellow-100 text-yellow-700'
              }`}
            >
              {ensuredProject.status === 'completed' ? 'Completed' : 'In Progress'}
            </span>
            
            {ensuredProject.featured && (
          ...
          <div className="flex flex-wrap gap-4">
            {ensuredProject.github && (
              <a
                href={ensuredProject.github}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-stone-900 text-stone-50 px-4 sm:px-6 py-2 sm:py-3 rounded-none hover:bg-stone-800 transition-all transform hover:scale-105 text-sm sm:text-base"
              >
            ...
            {ensuredProject.demo && (
              <a
                href={ensuredProject.demo}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-amber-400 text-stone-900 px-4 sm:px-6 py-2 sm:py-3 rounded-none hover:bg-amber-300 transition-all transform hover:scale-105 text-sm sm:text-base font-medium"
              >
                <ExternalLink className="w-4 h-4" />
                Live Demo
              </a>
            )}
            {(ensuredProject as any).slides && (
              <a
                href={(ensuredProject as any).slides}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-blue-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-none hover:bg-blue-600 transition-all transform hover:scale-105 text-sm sm:text-base font-medium"
              >
                <FileText className="w-4 h-4" />
                {(ensuredProject as any).slides.includes('youtube.com') ? 'Watch Video' : 'View Slides'}
              </a>
            )}
          </div>
        </motion.div>
        ...
        {ensuredProject.image && (
          ...
          >
            <div className="aspect-video bg-gradient-to-br from-amber-100 to-stone-200 border-2 border-stone-300 shadow-lg overflow-hidden rounded-lg">
              <img
                src={ensuredProject.image}
                alt={ensuredProject.title}
        ...
        >
          ...
          <div className="lg:col-span-2">
            <div className="bg-white/80 backdrop-blur-sm rounded-none border-l-4 p-4 sm:p-6 md:p-8 shadow-lg"
              ...
              {loading ? (
                ...
                <ReactMarkdown remarkPlugins={[remarkGfm]} className="prose prose-lg sm:prose-xl max-w-none text-stone-700">
                  {markdown}
                </ReactMarkdown>
              )}
            </div>
          ...
          <div className="space-y-6">
            {/* Tech Stack */}
            <div className="bg-stone-100 border-l-4 border-blue-400 p-4 sm:p-6 shadow-md">
              <h3 className="text-lg font-bold text-stone-900 mb-4 font-mono">Tech Stack</h3>
              <div className="space-y-2">
                {ensuredProject.tech.map((tech, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-blue-400 rounded-full" />
                    <span className="text-sm text-stone-700">{tech}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Links */}
            <div className="bg-stone-100 border-l-4 border-amber-400 p-4 sm:p-6 shadow-md">
              <h3 className="text-lg font-bold text-stone-900 mb-4 font-mono">Links</h3>
              <div className="space-y-3">
                {ensuredProject.github && (
                  <a
                    href={ensuredProject.github}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-stone-700 hover:text-blue-600 transition-colors"
                  >
                    <Github className="w-4 h-4" />
                ...
                {(ensuredProject as any).slides && (
                  ...
                  >
                    <FileText className="w-4 h-4" />
                    <span className="text-sm">{(ensuredProject as any).slides.includes('youtube.com') ? 'Demo Video' : 'Presentation'}</span>
                  </a>
                )}
              </div>
            </div>
